...
module Mutable =
    ...
    type MProvenance(__initial : Provenance.Provenance) =
        ...
        let _preview = MOption.Create(__initial.preview)
        
        member x.tree = _tree :> IMod<_>
        member x.highlight = _highlight :> IMod<_>
        member x.preview = _preview :> IMod<_>
        ...
        member x.Update(v : Provenance.Provenance) =
            if not (System.Object.ReferenceEquals(__current.Value, v)) then
                ...
                MOption.Update(_preview, v.preview)
    ...
    module Provenance =
        ...
        module Lens =
            ...
            let preview =
                { new Lens<Provenance.Provenance, Microsoft.FSharp.Core.Option<Aardvark.Base.ZTree<Provenance.Node>>>() with
                    override x.Get(r) = r.preview
                    override x.Set(r,v) = { r with preview = v }
                    override x.Update(r,f) = { r with preview = f r.preview }
                }
